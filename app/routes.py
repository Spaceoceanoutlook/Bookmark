...
def home():
    ...
    try:
        ...
        pinned_posts = session.query(Post).filter_by(pinned=True, user_id=current_user.id).order_by(desc(Post.id)).all()
    except Exception as e:
...
def save_topic():
    ...
    if topic_name:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to save topic: {str(e)}"})
        finally:
            session.close()
    else:
...
def edit_topic():
    ...
    if topic_id and topic_name:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to edit topic: {str(e)}"})
        finally:
            session.close()
    else:
...
def delete_topic():
    ...
    if topic_id:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to delete topic: {str(e)}"})
        finally:
            session.close()
    else:
...
def save_post():
    ...
    if post_content or post_photo:
        session = SessionLocal()
        try:
            new_post = Post(text=post_content or '', topic_id=topic_id, user_id=current_user.id)
            if post_photo:
            ...
            return jsonify({"success": True, "postId": new_post.id, "postContent": new_post.text or '', "photoFilename": new_post.photo})
        except Exception as e:
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to save post: {str(e)}"})
        finally:
            session.close()
    else:
        return jsonify({"success": False, "message": "Post content or photo is required"})
...
def delete_post():
    ...
    if post_id:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to delete post: {str(e)}"})
        finally:
            session.close()
    else:
...
def pin_post():
    ...
    if post_id:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to pin post: {str(e)}"})
        finally:
            session.close()
    else:
        return jsonify({"success": False, "message": "Post ID is required"})
...
def unpin_post():
    ...
    if post_id:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to unpin post: {str(e)}"})
        finally:
            session.close()
    else:
        return jsonify({"success": False, "message": "Post ID is required"})
...
def edit_post():
    ...
    if post_id:
        ...
        try:
            ...
            session.rollback()
            return jsonify({"success": False, "message": f"Failed to edit post: {str(e)}"})
        finally:
            session.close()
    else:
        return jsonify({"success": False, "message": "Post ID is required"})
