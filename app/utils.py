...
def get_title(url):
    user_agents = [
        ...
        "Safari/605.1.15",
        "Mozilla/5.0 (Linux; Android 10; SM-G973F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.77 Mobile "
        "Safari/537.36",
    ]
    headers = {
...
def link_search(t: str):
    ...
    for index, word in enumerate(words):
        if word.startswith("https"):
            words[index] = (
                f"<a href='{word}' target='_blank' rel='noopener noreferrer'>{get_title(word)}</a>"
            )
    return " ".join(words)
